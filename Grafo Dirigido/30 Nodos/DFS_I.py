...
class DFS_I:
    ...
    def ModeloDFSI(self):
        ...
        aristasNodosDFS=[]
        contadorDeVueltas=0
        p=DFS_I(self.nombreDelArchivo,self.nodoFuente)    
        ar=p.IdentificadorDeAristas()
        contador=ar[1]
        nN=self.nodoFuente
        while True:
            p=DFS_I(self.nombreDelArchivo,nN)#Se utiliza de nuevo la clase DFS
...
p=DFS_I('grafoMalla.gv',"id_1")
p.ModeloDFSI()

m=DFS_I('grafoErdosRenyi.gv','id_1')
m.ModeloDFSI()

n=DFS_I('grafoGilbert.gv',"id_1")
n.ModeloDFSI()

o=DFS_I('grafoGeografico.gv',"id_1")
o.ModeloDFSI()

q=DFS_I('grafoBarabasiAlbert.gv',"id_1")
q.ModeloDFSI()

r=DFS_I('grafoDorogovtsevMendes.gv',"id_1")
r.ModeloDFSI()