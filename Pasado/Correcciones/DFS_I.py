...
class DFS_I:
    ...
    def ModeloDFS_I(self):
        vectorDeNodosYaExplorados.append(self.nodoFuente)
        p=DFS_I(self.archivo,self.nodoFuente)    
        ar=p.LineasTotales()
        contador=p.MenuDeNodos(ar)
        nodosConectados=p.NodosConectados()
        nCSh=p.AleatorizarNodos(nodosConectados)
        nN=p.nuevoNodoFuente(nCSh)
        aristasNodosDFS=[]
        vectorUnidoDFS=Arista(self.nodoFuente,nN) #Aquí entra la clase arista
        aristasNodosDFS.append(vectorUnidoDFS.concatenar())

        #Para encontrar la rama principal 
        while True:
            p=DFS_I(self.archivo,nN)#Se utiliza de nuevo la clase DFS
        ...
        nombreArchivoAGenerar=nombreArchivoAGenerarP1+"DFSI.gv"
